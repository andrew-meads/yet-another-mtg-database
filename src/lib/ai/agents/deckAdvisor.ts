...
export const deckAdvisorPersona: AiAgentPersona = {
  ...
  buildSystemPrompt: (context) =>
    [
      ...
      "- Proposal semantics: an added copy comes from the user's active collection unless you set ephemeral (a deck-only placeholder — use it when the user doesn't own the card or just wants to sketch); a removed collection-backed copy returns to its collection, while a removed ephemeral placeholder is deleted; move relocates copies between the deck's sections and requires the destination sectionName.",
