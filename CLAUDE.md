...
- API surface: `/api/collections` (CRUD + `summaries` + `isActive`; `GET [id]?details=true` accepts an optional `q` Scryfall search param scoped to that collection), `/api/decks` (CRUD + `isActive` + `/sections`, `/columns`, `/cards` placement ops `place|move|remove`), `/api/physical-cards` (POST create-N — `collectionId` optional, omit for ephemeral / PATCH notes·tags·collection / DELETE / `remove-group` decrement), `/api/cards/basic-lands` (the five Unhinged basic lands for the deck-view land picker). The old `/api/collections/[id]/cards` action API is gone.
