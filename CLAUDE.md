...
- API surface: `/api/collections` (CRUD + `summaries` + `isActive`; `GET [id]?details=true` accepts an optional `q` Scryfall search param scoped to that collection), `/api/decks` (CRUD + `/sections`, `/columns`, `/cards` placement ops `place|move|remove`), `/api/physical-cards` (POST create-N / PATCH notes·tags·collection / DELETE / `remove-group` decrement). The old `/api/collections/[id]/cards` action API is gone.
