...
- **No-auth mode (`DISABLE_LOGIN=true`)**: a single env var makes the app run authentication-less, behaving as a fixed user `NO_AUTH_USER_ID = "000000000000000000000001"`. Server side, all session reads go through **`getAuthSession()`** (in `auth.ts`, replaces direct `getServerSession(authOptions)` calls everywhere) — when disabled it returns a synthetic `noAuthSession` and lazily/idempotently provisions the fixed user + an active "Main Collection" (memoized, mirrors the `signIn` auto-create via the shared `ensureMainCollection` helper). `src/proxy.ts` short-circuits its `getToken` 401 (env check inlined to keep mongoose out of the edge bundle). Client side, the flag is read server-side in the root layout and threaded via **`AuthModeContext`** (`useAuthMode()`); `AppBar` shows a "No-auth mode" indicator instead of the avatar, the login page redirects to `/search`, and the summary hooks (`useRetrieveCollection/DeckSummaries`) enable their queries on `disableLogin` too (since `useSession` is not authenticated).
