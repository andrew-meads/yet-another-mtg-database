...
Sorting is separate (`src/lib/sortConfig.ts`): some sort fields require a MongoDB aggregation pipeline (`useAggregation` + `buildAggregationSort`). `GET /api/cards` switches between `.find().sort()` and an aggregation pipeline, also using `$lookup` against `physicalcards` (localField `id` → foreignField `cardId`) for the `owned=true` filter. **Collection search is server-side too**: `GET /api/collections/[id]?details=true&q=...` runs `parseSearchQuery` intersected with the collection's `cardId`s **before** loading physical cards (the matching card ids are resolved first via `distinct("cardId")`, so only matching copies are ever fetched); the collection table still groups/sorts/virtualizes the returned cards client-side.
